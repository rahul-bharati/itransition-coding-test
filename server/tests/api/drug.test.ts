...
describe("Drug API", () => {
    describe("GET /drug - Basic retrieval", () => {
        it("should return all drugs with default sorting (newest first) when limit is large", async () => {
            const resp = await request(server.app).get("/api/drug?limit=9999");
        ...
        it("should return default-limited page when limit is not specified", async () => {
            const resp = await request(server.app).get("/api/drug");
            expect(resp.status).toBe(200);
            const { items, totalItems } = resp.body;
            expect(Array.isArray(items)).toBe(true);
    ...
    describe("GET /drug - Pagination with page", () => {
        it("should return first 5 items with limit=5", async () => {
            const resp = await request(server.app).get("/api/drug?limit=5");
            expect(resp.status).toBe(200);
            const { items, totalItems } = resp.body;
        ...
        it("should return first 10 items with limit=10", async () => {
            const resp = await request(server.app).get("/api/drug?limit=10");
            expect(resp.status).toBe(200);
            const { items, totalItems } = resp.body;
        ...
        it("should return page 2 with limit=5", async () => {
            const page2 = await request(server.app).get("/api/drug?limit=5&page=2");
            expect(page2.status).toBe(200);
            expect(page2.body.items.length).toBe(5);
            expect(page2.body.totalItems).toBe(DRUGS_TOTAL_COUNT);

            const page1 = await request(server.app).get("/api/drug?limit=5&page=1");
        ...
        it("should return page 3 with limit=5", async () => {
            const resp = await request(server.app).get("/api/drug?limit=5&page=3");
            expect(resp.status).toBe(200);
        ...
        it("should return page 2 with limit=10", async () => {
            const resp = await request(server.app).get("/api/drug?limit=10&page=2");
            expect(resp.status).toBe(200);
        ...
        it("should treat page=1 the same as default for a given limit", async () => {
            const resp1 = await request(server.app).get("/api/drug?limit=5&page=1");
            const resp2 = await request(server.app).get("/api/drug?limit=5");
        ...
        it("should return empty array when requesting a page beyond available data", async () => {
            const resp = await request(server.app).get("/api/drug?limit=10&page=100");
            expect(resp.status).toBe(200);
    ...
    describe("GET /api/drug - Sorting", () => {
        it("should sort by launchDate ascending", async () => {
            const resp = await request(server.app).get("/api/drug?sortBy=launchDate&sortOrder=asc&limit=9999");
            expect(resp.status).toBe(200);
        ...
        it("should sort by launchDate descending (explicit)", async () => {
            const resp = await request(server.app).get("/api/drug?sortBy=launchDate&sortOrder=desc&limit=9999");
            expect(resp.status).toBe(200);
            const normalizedItems = stripRuntimeFields(resp.body.items);
            expect(normalizedItems).toEqual(DRUGS_DEFAULT);
        });
        ...
        it("should sort by genericName ascending", async () => {
            const resp = await request(server.app).get("/api/drug?sortBy=genericName&sortOrder=asc&limit=9999");
            expect(resp.status).toBe(200);
            const normalizedItems = stripRuntimeFields(resp.body.items);
        ...
        it("should sort by genericName descending", async () => {
            const resp = await request(server.app).get("/api/drug?sortBy=genericName&sortOrder=desc&limit=9999");
            expect(resp.status).toBe(200);
            const normalizedItems = stripRuntimeFields(resp.body.items);
        ...
        it("should sort by company ascending (sanity check with comparator)", async () => {
            const resp = await request(server.app).get("/api/drug?sortBy=company&sortOrder=asc&limit=9999");
            expect(resp.status).toBe(200);
    ...
    describe("GET /api/drug - Filtering by company", () => {
        it("should filter by CardioMed Pharmaceuticals", async () => {
            const resp = await request(server.app).get("/api/drug?company=CardioMed Pharmaceuticals&limit=9999");
            expect(resp.status).toBe(200);
            const { items, totalItems } = resp.body;
        ...
        it("should filter by MindCare Pharmaceuticals", async () => {
            const resp = await request(server.app).get("/api/drug?company=MindCare Pharmaceuticals&limit=9999");
            expect(resp.status).toBe(200);
            const { items, totalItems } = resp.body;
        ...
        it("should return empty array for non-existent company", async () => {
            const resp = await request(server.app).get("/api/drug?company=NonExistentCompany");
            expect(resp.status).toBe(200);
            expect(resp.body.items.length).toBe(0);
        ...
        it("should combine company filter with pagination", async () => {
            const resp = await request(server.app).get("/api/drug?company=CardioMed Pharmaceuticals&limit=2");
            expect(resp.status).toBe(200);
    ...
    describe("GET /api/drug - Search functionality", () => {
        it('should search for "amox" across fields', async () => {
            const resp = await request(server.app).get("/api/drug?search=amox&limit=9999");
            expect(resp.status).toBe(200);
            const normalizedItems = stripRuntimeFields(resp.body.items);
        ...
        it('should search for "Metformin" (case-insensitive)', async () => {
            const resp = await request(server.app).get("/api/drug?search=Metformin&limit=9999");
            expect(resp.status).toBe(200);
            const normalizedItems = stripRuntimeFields(resp.body.items);
        ...
        it("should return empty array for no matches", async () => {
            const resp = await request(server.app).get("/api/drug?search=XYZ999NotFound");
            expect(resp.status).toBe(200);
            expect(resp.body.items.length).toBe(0);
        });

        it("should combine search with pagination", async () => {
            const resp = await request(server.app).get("/api/drug?search=a&limit=5&page=1");
            expect(resp.status).toBe(200);
            expect(resp.body.items.length).toBeLessThanOrEqual(5);
        });
    });

    describe("GET /api/drug - Combined filters", () => {
        it("should combine company filter and search", async () => {
            const resp = await request(server.app).get("/api/drug?company=CardioMed Pharmaceuticals&search=l&limit=9999");
            expect(resp.status).toBe(200);
        ...
        it("should combine sorting, filtering, and pagination", async () => {
            const resp = await request(server.app)
                .get("/api/drug?company=CardioMed Pharmaceuticals&sortBy=genericName&sortOrder=asc&limit=2&page=1");
            expect(resp.status).toBe(200);
            expect(resp.body.items.length).toBeLessThanOrEqual(2);
            resp.body.items.forEach((drug: any) => {
                expect(drug.company).toBe("CardioMed Pharmaceuticals");
    ...
    describe("GET /api/drug - Error handling", () => {
        it("should handle invalid limit gracefully", async () => {
            const resp = await request(server.app).get("/api/drug?limit=invalid");
            expect(resp.status).toBe(400);
        });

        it("should handle invalid page gracefully", async () => {
            const resp = await request(server.app).get("/api/drug?page=invalid");
            expect(resp.status).toBe(400);
        });
        ...
        it("should handle negative page", async () => {
            const resp = await request(server.app).get("/api/drug?page=-1");
            expect(resp.status).toBe(400);
        });
    });


    describe("GET /api/drug/companies - Get distinct companies", () => {
        it('should return list of all unique companies', async () => {
            const resp = await request(server.app).get("/api/drug/companies");
            expect(resp.status).toBe(200);
