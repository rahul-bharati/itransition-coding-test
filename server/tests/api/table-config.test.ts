...
describe('Table Config test', () => {
    ...
    it("GET /api/table-config", async () => {
        const tableConfig = DEFAULT_TABLE_CONFIG

        const response = await request(server.app).get("/api/table-config");
        expect(response.status).toBe(200);
        // @ts-ignore
        expect(response.body as ITableConfig).toMatchObject(tableConfig)
    })

    it("GET /api/table-config - after initial call", async () => {
        const tableConfig = DEFAULT_TABLE_CONFIG

        await request(server.app).get("/api/table-config");
        const response = await request(server.app).get("/api/table-config");
        expect(response.status).toBe(200);
        // @ts-ignore
        expect(response.body as ITableConfig).toMatchObject(tableConfig)
    ...
    it("PATCH /api/table-config - throws error when config is missing", async () => {
        const response = await request(server.app)
            .patch("/api/table-config")
            .send(null);

        expect(response.status).toBe(400);
    })

    it("PATCH /api/table-config", async () => {
